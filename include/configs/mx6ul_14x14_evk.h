...
#define CONFIG_MFG_NAND_PARTITION "mtdparts=gpmi-nand:4m(boot),1m(dtb),16m(kernel),4m(logo),153(rootfs),-(opt)"
#else
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bootargs=console=ttymxc0,115200 ubi.mtd=3 "  \
		...
		CONFIG_BOOTARGS_CMA_SIZE \
		"mtdparts=gpmi-nand:4m(boot),1m(dtb),16m(kernel),4m(logo),153(rootfs),-(opt)\0"\
